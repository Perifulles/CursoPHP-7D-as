<?php
// ═══════════════════════════════════════
//  BLOQUE 1 — Conexión PDO (patrón Vicente)
// ═══════════════════════════════════════
//
// Como no tenemos MySQL en Docker, usamos SQLite para que el código
// realmente funcione. PERO el patrón de clase Conexion, config.json,
// DSN, getConn(), query() y prepare() es IDÉNTICO al de Vicente.
// En el examen cambiarías "sqlite:..." por el DSN de MySQL.
//
// Helper: crea una BD SQLite con datos de prueba
function crearBdPrueba() {
    $db = new PDO("sqlite::memory:");
    $db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $db->exec("CREATE TABLE productos (id INTEGER PRIMARY KEY AUTOINCREMENT, nombre TEXT, precio REAL)");
    $db->exec("INSERT INTO productos (nombre, precio) VALUES ('Teclado', 29.99)");
    $db->exec("INSERT INTO productos (nombre, precio) VALUES ('Ratón', 15.50)");
    $db->exec("INSERT INTO productos (nombre, precio) VALUES ('Monitor', 249.00)");
    $db->exec("INSERT INTO productos (nombre, precio) VALUES ('Auriculares', 45.00)");
    $db->exec("INSERT INTO productos (nombre, precio) VALUES ('Webcam', 62.30)");
    return $db;
}

ob_start();

echo "<h3>Ejercicio 1.1 — El archivo config.json</h3>";
// En el patrón de Vicente, los datos de conexión se guardan en un JSON.
...
//
// En el constructor:
//   PASO 1: Leer el JSON → $configData = file_get_contents("config.json");
//           (como no tenemos archivo real, simula con un string JSON)
...
//
// NOTA: Como no tenemos MySQL, usa crearBdPrueba() en vez de new PDO($dsn,...).
// El PATRÓN es lo mismo, solo cambia el DSN.
// En el examen escribirías el DSN de MySQL tal cual.
//
// $conexion = new Conexion();
...
// Usando la Conexion del ejercicio anterior, haz una consulta DIRECTA
// (sin prepare, sin parámetros). Así es como lo enseña Vicente primero:
//
...
// Ahora haz una consulta PREPARADA con marcadores ? (posicionales):
//
...
<h1>🗄️ Conexión PDO (patrón Vicente)</h1>
...
<div class="info"><b>💡 SQLite para probar:</b> Como no tenemos MySQL en Docker, usamos SQLite. El código de la clase Conexion y las consultas es IDÉNTICO; solo cambia el DSN. En el examen escribes el DSN de MySQL.</div>
...
<p>Amplía Conexion: después de guardar las propiedades, construye el DSN (<code>mysql:host=...;dbname=...;charset=utf8mb4</code>) y crea <code>$this->conn = new PDO($dsn, ...)</code>. Getter <code>getConn()</code>. (Usa <code>crearBdPrueba()</code> para probar.)</p>
</div>
...
<hr class="separador">
<div class="tip"><b>✅ Resumen de los 5 pasos:</b> (1) Leer JSON config, (2) guardar propiedades, (3) construir DSN, (4) new PDO con el DSN, (5) consultas — primero "a piñón" con query(), después preparadas con prepare()+execute(). Dos estilos de prepare: con <code>?</code> (cuaderno) y con <code>:nombre</code> + bindValue (examen).</div>
